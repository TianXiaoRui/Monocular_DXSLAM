...
//add trackMonocular
                           ...
                           const cv::Mat &global_desc)
    {
       ...
       // cv::Mat Tcw = mpTracker->GrabImageRGBD(im, depthmap, timestamp, keypoints, local_desc, global_desc);
        cv::Mat Tcw = mpTracker->GrabImageMonocular(im,timestamp, keypoints, local_desc, global_desc);
...
//todo modified the save trajectory to each frames.
    ...
    void System::SaveTrajectoryTUM(const string &filename)
    {
        ...
        list<DXSLAM::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();
